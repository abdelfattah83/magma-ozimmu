    ...
    magma_event_t events[2], magma_queue_t queues[2] )
{
    ...
    if (mode == MagmaHybrid) {
        if ( magma_zgetrf_gpu_recommend_cpu(m, n, nb) ) {
            ...
            ldwork = maxm;
            h_workspace_bytes += nb * ldwork * sizeof(magmaDoubleComplex); // dAP on CPU
            d_workspace_bytes += nb * maxm * sizeof(magmaDoubleComplex);   // dAP on GPU
            if( !(m == n) ) {
        ...
        // native workspace
        d_workspace_bytes += nb * maxm * sizeof(magmaDoubleComplex); // dAP
